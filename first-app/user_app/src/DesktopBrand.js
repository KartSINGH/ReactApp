...
import {desktopBrandSelected} from './Actions/index.js';
class DesktopBrand extends Component{
    state={
        data:[],
    }
    componentWillMount() {
        ...
        axios({
            method: 'get',
            url :'https://api.shuttlescrap.com/desktop/get_count', 
        })
        .then((data1)=>{
            this.setState({
                data: data1.data
    ...
    select_brand(brand){
        ...
        this.props.desktopBrandSelected(brand);
    }
    renderBrands(){
            ...
            return(
                ...
                <Card style={{flex:0.5,width:100}}>
                    <CardSection style={{flex:0.5}}>
                        <Text style={{fontSize:15,lineHeight:20}}>{options[0]+" desktops"}</Text>
                    </CardSection>
                </Card>
                </View>
            )
...
export default connect(null,{desktopBrandSelected})(DesktopBrand);