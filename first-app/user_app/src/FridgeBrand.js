...
import {fridgeBrandSelected} from './Actions/index.js';
class FridgeBrand extends Component{
    state={
        data:[],
    }
    componentWillMount() {
        ...
        axios({
            method: 'get',
            url :'https://api.shuttlescrap.com/fridge/get_count', 
        })
        .then((data1)=>{
            this.setState({
                data: data1.data
    ...
    select_brand(brand){
        ...
        this.props.fridgeBrandSelected(brand);
    }
    renderBrands(){
            ...
            return(
                ...
                <Card style={{flex:0.5,width:100}}>
                    <CardSection style={{flex:0.5}}>
                        <Text style={{fontSize:15,lineHeight:20}}>{options[0]+" fridge"}</Text>
                    </CardSection>
                </Card>
                </View>
            )
...
export default connect(null,{fridgeBrandSelected})(FridgeBrand);