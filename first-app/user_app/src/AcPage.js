...
class AcPage extends Component{
    state={
        data:[],
    }
    componentDidMount() {
        axios({
            method:'post',
            url:'https://api.shuttlescrap.com/ac/get_single_ac',
            data:{
                id:this.props.ac_id
            }
        }).then((resp)=>{
            ...
            return(
                ...
                <Card style={{flexDirection:'column'}}>
                    <CardSection style={{flexDirection:'column',padding:15}}>
                       <Text style={{fontSize:15,marginBottom:15,textAlign:'center',alignSelf:'center',color:'#22aa86'}}> Looking to sell this AC to Shuttlescrap?Get upto Rs.{show} from us!</Text>
                    </CardSection>
                </Card>
...
export default connect(mapStateToProps,{acSelected})(AcPage);