...
import {tvBrandSelected} from './Actions/index.js';
class TvBrand extends Component{
    state={
        data:[],
    }
    componentWillMount() {
        ...
        axios({
            method: 'get',
            url :'https://api.shuttlescrap.com/tv/get_count', 
        })
        .then((data1)=>{
            this.setState({
                data: data1.data
    ...
    select_brand(brand){
        ...
        this.props.tvBrandSelected(brand);
    }
    renderBrands(){
            ...
            return(
                ...
                <Card style={{flex:0.5,width:100}}>
                    <CardSection style={{flex:0.5}}>
                        <Text style={{fontSize:15,lineHeight:20}}>{options[0]+" TV"}</Text>
                    </CardSection>
                </Card>
                </View>
            )
...
export default connect(null,{tvBrandSelected})(TvBrand);