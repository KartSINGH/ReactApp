...
class MicrowaveList extends Component{
    state={
        data:[],
    }
    componentDidMount() {
        axios({
            method:'post',
            url:'https://api.shuttlescrap.com/microwave/get_microwave_from_brand',
            data:{
                brand_name:this.props.microwave_brand
            }
        }).then((resp)=>{
            this.setState({
                data: resp.data
    ...
    getDevice(id){
        this.props.microwaveSelected(id);
    }
    renderBrands(){
...
mapStateToProps=({microwave})=>{
    const {microwave_brand} = microwave;
    return{
        microwave_brand
    }
}
export default connect(mapStateToProps,{microwaveSelected})(MicrowaveList);