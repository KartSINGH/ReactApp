...
class FridgeList extends Component{
    ...
    componentDidMount() {
        axios({
            method:'post',
            url:'https://api.shuttlescrap.com/fridge/get_fridge_from_brand',
            data:{
                brand_name:this.props.fridge_brand
            }
        }).then((resp)=>{
            this.setState({
                data: resp.data
...
mapStateToProps=({fridge})=>{
    const {fridge_brand} = fridge;
    return{
        fridge_brand
    }
}
export default connect(mapStateToProps,{fridgeSelected})(FridgeList);