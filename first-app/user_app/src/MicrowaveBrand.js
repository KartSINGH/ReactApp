...
import {microwaveBrandSelected} from './Actions/index.js';
class MicrowaveBrand extends Component{
    state={
        data:[],
    }
    componentWillMount() {
        ...
        axios({
            method: 'get',
            url :'https://api.shuttlescrap.com/microwave/get_count', 
        })
        .then((data1)=>{
            this.setState({
                data: data1.data
    ...
    select_brand(brand){
        ...
        this.props.microwaveBrandSelected(brand);
    }
    renderBrands(){
            ...
            return(
                ...
                <Card style={{flex:0.5,width:100}}>
                    <CardSection style={{flex:0.5}}>
                        <Text style={{fontSize:15,lineHeight:20}}>{options[0]+" microwave"}</Text>
                    </CardSection>
                </Card>
                </View>
            )
...
export default connect(null,{microwaveBrandSelected})(MicrowaveBrand);