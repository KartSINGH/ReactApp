...
import {microwaveSelected} from './Actions/index.js';
class MicrowavePage extends Component{
    state={
        data:[],
    }
    componentDidMount() {
        axios({
            method:'post',
            url:'https://api.shuttlescrap.com/microwave/get_single_microwave',
            data:{
                id:this.props.microwave_id
            }
        }).then((resp)=>{
            this.setState({
                data: resp.data
            ...
            return(
                ...
                <Card style={{flexDirection:'column'}}>
                    <CardSection style={{flexDirection:'column',padding:15}}>
                       <Text style={{fontSize:15,marginBottom:15,textAlign:'center',alignSelf:'center',color:'#22aa86'}}> Looking to sell this TV to Shuttlescrap?Get upto Rs.{show} from us!</Text>
...
mapStateToProps=({microwave})=>{
    const {microwave_id} = microwave;
    return{
        microwave_id
    }
}
export default connect(mapStateToProps,{microwaveSelected})(MicrowavePage);